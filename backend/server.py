...
# article on /tcf-canada-vocabulary. Generated by tools/lead_pdf_preview.py.
#
# The free pages ship inside the frontend bundle, which is where anything
# public belongs. These live here for the reason they are not there: the
# bundle is served to anybody who asks, so a page put in it is a page with no
# account behind it whatever the component says.
VOCAB_GATED_CONTENT = DOWNLOADS_DIR / "vocab-guide-gated.json"
VOCAB_TOTAL_PAGES = 25
VOCAB_FIRST_GATED = 5


@app.get("/api/downloads/tcf-vocabulary/content")
async def vocabulary_content(user: Optional[User] = Depends(get_optional_user)):
    """Pages 5 to 24 of the guide, block by block, to somebody with an account.

    Signed out, the page shows a blurred teaser and never asks for this — but
    the check is here and not there, because a gate the browser applies is a
    gate anybody can walk around by typing the address.

    Cached privately: the file changes only when the PDF does, and `private`
    is what stops a shared proxy handing a copy to the next visitor, who may
    have no account.
    """
    if user is None:
        raise HTTPException(
    ...
    if not VOCAB_GATED_CONTENT.is_file():
        log.error("Vocabulary guide text is missing from %s", VOCAB_GATED_CONTENT)
        raise HTTPException(status_code=404, detail="Guide not available")
    return FileResponse(VOCAB_GATED_CONTENT, media_type="application/json",
