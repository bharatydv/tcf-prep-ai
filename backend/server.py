...
# The pages of the vocabulary guide, as pictures, for the reader on
# /tcf-canada-vocabulary. Generated by tools/lead_pdf_preview.py.
#
# These live here rather than in frontend/public for the reason everything in
# frontend/public is not here: that directory is served to anybody who asks,
# so a page put there is a page with no account behind it. The free pages
# genuinely are public and are files there; these are the rest.
VOCAB_PAGES_DIR = DOWNLOADS_DIR / "vocab-pages"
VOCAB_TOTAL_PAGES = 25
VOCAB_FIRST_GATED = 5


@app.get("/api/downloads/tcf-vocabulary/pages/{number}")
async def vocabulary_page(number: int,
                          user: Optional[User] = Depends(get_optional_user)):
    """One sharp page of the guide, to somebody with an account.

    Signed out, the reader shows the blurred copies from frontend/public
    instead and never asks for these — but the check is here and not there,
    because a gate the browser applies is a gate anybody can walk around by
    typing the address.

    Cached privately and for a long time: a page of a PDF that is regenerated
    only when the PDF changes, and the `private` is what stops a shared proxy
    keeping a copy to hand to the next visitor, who may have no account.
    """
    if not 1 <= number <= VOCAB_TOTAL_PAGES:
        raise HTTPException(status_code=404, detail="No such page")
    if user is None:
        raise HTTPException(
            status_code=403,
            detail="Create a free account to read the rest of the guide.")
    path = VOCAB_PAGES_DIR / f"page-{number}.webp"
    if not path.is_file():
        log.error("Vocabulary page %s is missing from %s", number, VOCAB_PAGES_DIR)
