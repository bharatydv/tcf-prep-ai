    ...
    <div className="overflow-x-clip bg-white">
      ...
      <footer className="bg-ink text-violet-200/70" style={{ background: '#120822' }}>
        <div className="mx-auto grid max-w-7xl gap-10 px-4 py-14 sm:px-6 lg:grid-cols-[1.4fr_1fr_1fr_1.2fr]">
          ...
          <div>
            <p className="font-heading text-sm font-bold text-white">{t('land.footProduct')}</p>
            <ul className="mt-4 space-y-2.5 text-xs">
              <li><Link to="/practice" className="transition hover:text-white">{t('land.footWriting')}</Link></li>
              <li><Link to="/exam-simulator" className="transition hover:text-white">{t('land.footSimulator')}</Link></li>
              <li><Link to="/exam/reading-comprehension" className="transition hover:text-white">{t('land.footMock')}</Link></li>
              <li><Link to="/speaking" className="transition hover:text-white">{t('land.footSpeaking')}</Link></li>
            </ul>
          </div>
          <div>
            <p className="font-heading text-sm font-bold text-white">{t('land.footResources')}</p>
            <ul className="mt-4 space-y-2.5 text-xs">
              <li><Link to="/recent-topics" className="transition hover:text-white">{t('land.footRecent')}</Link></li>
              <li><Link to="/review" className="transition hover:text-white">{t('land.footReview')}</Link></li>
              <li><Link to="/pricing" className="transition hover:text-white">{t('land.footPacks')}</Link></li>
              <li><Link to="/dashboard" className="transition hover:text-white">{t('land.footDashboard')}</Link></li>
            </ul>
          </div>
          <div>
            ...
            <form className="mt-3 flex overflow-hidden rounded-xl bg-white/10 ring-1 ring-white/15"
              onSubmit={(e) => { e.preventDefault(); if (!email.trim()) return; toast.success(t('land.subscribed')); setEmail(''); }}>
              <input value={email} onChange={(e) => setEmail(e.target.value)} type="email" placeholder={t('land.emailPlaceholder')}
              ...
              <button className="bg-gradient-to-r from-primary to-fuchsia-600 px-4 text-white transition hover:brightness-110" aria-label={t('land.subscribe')} data-testid="newsletter-button">
